$(function(){
	...
	function setupNotifier() {
		...
		if (!$lrContainer.length) {
			$lrContainer = $("<div></div>").attr('id', 'TPG_lRNotifier').attr('aria-live', 'polite').attr("aria-relevant", "additions").attr("role", "status").addClass("HiddenText").appendTo(document.body);
		}
	}
